...
function setup() {
  ...
  //for (let i = 108; i < 132; i++) {
    ...
    let url = 'https://raw.githubusercontent.com/raywenderlich/recipes/master/Recipes.json';
...
function process(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    titleMarkov.feed(data[i].name);
    let elevator = data[i].steps;
    let test =""
    for(let j=0; j<elevator.length; j++) {
      test += elevator[j]
    }
    console.log(test);
